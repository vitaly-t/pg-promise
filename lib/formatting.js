    ...
    switch (typeof value) {
        ...
        default:
            ...
            if (value instanceof Array) {
                return $to.array(value, options && options.cap);
            }
...
function formatArray(array, cap) {
    const loop = a => '[' + a.map(value => value instanceof Array ? loop(value) : formatValue({value})).join() + ']';
    const prefix = cap ? 'ARRAY' : 'array';
...
function formatCSV(values) {
    ...
    if (typeof values === 'object' && values !== null) {
        return Object.keys(values).map(v => formatValue({value: values[v]})).join();
    }
    return values === undefined ? '' : formatValue({value: values});
}
...
const formatAs = {
    ...
    array(query, array, raw, options) {
        ...
        return query.replace(npm.patterns.multipleValues, name => {
            ...
            if (idx < array.length) {
                return formatValue({value: array[idx], fm: v.fm});
            }
            if ('def' in options) {
                const d = options.def, value = typeof d === 'function' ? d.call(array, idx, array) : d;
                return formatValue({value, fm: v.fm});
            }
            if (options.partial) {
                return name;
            }
...
const $as = {
     ...
     *
     * @param {boolean} [options.cap=false]
     * When `true`, uses `ARRAY` instead of `array`.
     *
     * @returns {string}
     */
    array(arr, options) {
        options = assertOptions(options, ['cap']);
        arr = resolveFunc(arr);
        if (isNull(arr)) {
            return 'null';
        }
        if (arr instanceof Array) {
            return $to.array(arr, options.cap);
        }
    ...
    format(query, values, options) {
        options = assertOptions(options, ['cap', 'partial', 'def']);
