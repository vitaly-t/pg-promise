...
function $formatQuery(query, values) {
    ...
    if (Array.isArray(values)) {
        ...
        if (values && typeof(values) === 'object' && !(values instanceof Date)) {
            ...
            var names = enumVars(query);
            for (var i = 0; i < names.length; i++) {
                var rawValue = values[names[i]];
                var value = wrapValue(rawValue);
                if (value === null) {
                    throw new Error("Cannot convert type '" + typeof(rawValue) + "' into a query variable value.");
                }
                var pattern = "\\$\\{\\s*" + names[i] + "\\s*}";
                query = query.replace(new RegExp(pattern, 'g'), value);
            }
        } else {
            if (values !== undefined) {
