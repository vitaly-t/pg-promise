...
// It can only fail when a function-parameter
// throws an error, which is not handled.
...
//
// 1. format "$1, $2, etc", when 'values' is string, boolean, number, date, function or null
//    (or an array of the same types, plus undefined);
...
var $as = {
    ...
    number: function (num) {
        ...
        if (typeof(num) !== 'number') {
            throw new Error(wrapText(num) + " doesn't represent a valid number.");
        }
        if (isFinite(num)) {
            return num.toString();
        }
    ...
    func: function (func, raw) {
        ...
        if (typeof(func) !== 'function') {
            throw new Error(wrapText(func) + " doesn't represent a valid function.");
        }
    ...
    json: function (obj, raw) {
        ...
        return raw ? s : wrapText(s.replace(/'/g, "''"));
    },
    csv: function (values) {
