...
// Regular and raw-text variables are returned independently.
//
...
function enumVars(txt) {
    var v, names = [];
    var vnp = "\\s*[a-z\\$_][a-z0-9\\$_]*\\^?\\s*"; // variable name pattern;
    var reg = new RegExp("\\$\\{" + vnp + "}|\\$\\(" + vnp + "\\)", 'gi');
...
var formatAs = {
    ...
    object: function (query, obj) {
        ...
        for (var i = 0; i < names.length; i++) {
            ...
            var vnp = "\\$\\{\\s*" + name + "\\s*}|\\$\\(\\s*" + name + "\\s*\\)";
            // Replacing all occurrences of the 'name' variable with its 'value':
            query = query.replace(new RegExp(vnp, 'g'), v);
        }
        return query;
    },
