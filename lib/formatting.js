...
// Supported types: text, boolean, date, number, null,
// undefined and array.
function formatValue(val, strip) {
    if (isDBNull(val)) {
        return 'null';
    }
    switch (typeof(val)) {
        case 'string':
            return $as.text(val, strip);
        case 'boolean':
            return $as.bool(val);
        case 'function':
...
// Converts array into Postgres format: http://www.postgresql.org/docs/9.4/static/arrays.html
...
// optional ${varName^} for injecting a stripped value.
//
...
var $as = {
    text: function (txt, strip) {
        ...
        // replacing each single-quote symbol with two;
        var s = txt.replace(/'/g, "''");

        return strip ? s : wrapText(s);
    },
    bool: function (val) {
        if (isDBNull(val)) {
            return 'null';
...
//
// Value stripping is supported via syntax: $1^,$2^ or ${propName^},
// so any text/date returned isn't wrapped in single quotes.
//
...
function $formatQuery(query, values) {
    ...
    if (Array.isArray(values)) {
        ...
        for (var i = 0; i < values.length; i++) {
            ...
            // check for stripped variables presence:
            var strip = query.search(new RegExp(pattern + '\\^')) !== -1;
            var value = formatValue(values[i], strip);
            if (value === null) {
                throw new Error(errors.array(values[i], i));
            }
            if (strip) {
        ...
        if (values && typeof(values) === 'object' && !(values instanceof Date)) {
            ...
            for (var i = 0; i < names.length; i++) {
                ...
                var strip = name[name.length - 1] === '^'; // strip flag;
                var sName = strip ? name.substring(0, name.length - 1) : name; // stripped name;
                if (!(sName in values)) {
                ...
                var value = formatValue(prop, strip);
                if (value === null) {
            ...
            if (values !== undefined) {
                ...
                //var stripPattern = /\$1(?!\d)\^/
                var strip = query.search(/\$1(?!\d)\^/) !== -1;
                var value = formatValue(values, strip);
                if (value === null) {
                    // error: not a simple value;
                    throw new Error(errors.param(values));
                }
