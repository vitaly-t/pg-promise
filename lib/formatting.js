...
const $as = {
     ...
     *
     * **Added in v7.3.0:** The surrounding double quotes are not added when the alias is a same-case single word, even if it
     * contains digits (but no leading digits).
     *
    ...
    alias(name) {
        ...
        if (name && typeof name === 'string') {
            const m = name.match(/[a-z][a-z0-9]*|[A-Z][A-Z0-9]*/);
            if (m && m[0] === name) {
                return name;
            }
            return '"' + name.replace(/"/g, '""') + '"';
