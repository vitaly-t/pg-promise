 ...
 *
 * @param {function} cb
 * A callback function that takes the following arguments:
 *
 * - `value` - property value
 * - `name` - property name
 * - `obj` - current object (which contains the property)
...
function mapProp(obj, cb) {
    ...
    function loop(obj, dest, cb, level) {
        var index = 0;
        for (var prop in obj) {
            var val = obj[prop];
            if (val && typeof val === 'object') {
                dest[prop] = {};
                loop(val, dest[prop], cb, level + 1);
            } else {
                dest[prop] = cb(val, prop, obj, dest, level, index);
            }
            index++;
        }
    }
