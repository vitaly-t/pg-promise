...
function _enumSql(dir, options, cb, namePath) {
    ...
    fs.readdirSync(dir).forEach(function (file) {
        var stat, fullPath = path.join(dir, file);
        try {
...
function buildSqlModule(config) {
    ...
    if (utils.isText(config)) {
        config = require(path.join(utils.startDir, config));
    } else {
        if (utils.isNull(config)) {
            var defConfig = path.join(utils.startDir, 'sql-config.json');
