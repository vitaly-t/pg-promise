 ...
 * @augments Error
 * @summary Query Result Error class.
 * @description
 *
...
QueryResultError.prototype.toString = function (level) {
    ...
    var lines = [
        'QueryResultError {',
        gap + 'code: queryResultErrorCode.' + errorMessages[this.code].name,
        gap + 'message: "' + this.message + '"',
        gap + 'received: ' + this.received,
        gap + 'query: ' + (typeof this.query === 'string' ? '"' + this.query + '"' : JSON.stringify(this.query))
    ];
