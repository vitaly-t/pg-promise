...
PreparedStatement.prototype.parse = function () {
    ...
    if (!$npm.utils.isText(this.name)) {
        return new PSE("Invalid 'name' in PreparedStatement. It must be a non-empty text string.");
    }
    ps.name = this.name;
    if (this.text instanceof $npm.QueryFile) {
        ...
        qf.prepare();
        if (qf.error) {
            return new PSE("QueryFile error in PreparedStatement '" + this.name + "':" + EOL + "    " + qf.error.message);
        }
        ps.text = qf.query;
    } else {
        ps.text = this.text;
