...
function ColumnSet(columns, options) {
    ...
    /**
     * @method helpers.ColumnSet#setExcluded
     * @description
     * ** Added in v7.3.0 **
     *
     * Generates excluded assignments for all columns in the set.
     *
     * It is normally appended to an `ON CONFLICT` query clause.
     *
     * @param {Object} options
     * Optional Parameters.
     *
     * @param {string | Array<string>} options.skip
     * Name(s) of the column(s) to be skipped (case-sensitive).
     *
     ...
     *
     * cs.setExcluded({skip: 'id'})
     * //=> "city"=excluded."city","street"=excluded."street"
     *
     */
    this.setExcluded = options => {
        const skip = options && ((typeof options.skip === 'string' && [options.skip]) || (Array.isArray(options.skip) && options.skip));
        let cols = this.columns;
        if (skip) {
            cols = cols.filter(c => skip.indexOf(c.name) === -1);
        }
        return cols.map(c => c.escapedName + '=excluded.' + c.escapedName).join();
    };
