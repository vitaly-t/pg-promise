...
function ColumnSet(columns, options) {
     ...
     */
    this.assign = options => {
        var hasAlias = options && options.alias && typeof options.alias === 'string';
        if (updates && !hasAlias) {
            return updates;
        }
        var dynamic = hasAlias,
        ...
        var alias = hasAlias ? $npm.formatting.as.alias(options.alias) + '.' : '';
        list = list.map(c => alias + c.escapedName + '=' + c.variable + c.castText).join();
