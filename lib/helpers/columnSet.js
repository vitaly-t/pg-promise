 ...
 * @description
 *
 * Performance-optimized, read-only structure with all query-formatting columns.
 *
...
function ColumnSet(columns, options) {
    ...
    var inherit, names, variables, updates, canUpdateMany;
    ...
    this.canUpdate = function (data) {
        ...
        if (Array.isArray(data)) {
            if (canUpdateMany === undefined) {
                cnd = this.columns.$countIf(function (c) {
                    return c.cnd;
                });
                canUpdateMany = total > cnd;
            }
            return canUpdateMany && data.length > 0;
        }
