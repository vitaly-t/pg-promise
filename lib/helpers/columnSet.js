...
function ColumnSet(columns, options) {
    ...
    /**
     * @name helpers.ColumnSet#cndCount
     * @private
     * @type Number
     * @readonly
     * @description
     ...
     */
    Object.defineProperty(this, 'cndCount', {
        get: function () {
            return cndCount;
        }
    });
    ...
    /**
     * @method helpers.ColumnSet.canGenerate
     * @description
     * Checks if it is possible to generate a query for the specified `data` and the current {@link helpers.ColumnSet ColumnSet},
     * without running into errors related to the shortage of effective columns and/or lack of data.
     *
     * The result and the logic depend on whether `data` is a single object or an array, and it is consistent with the validation
     * logic that's implemented by all the query-generating methods.
     *
     * This method is mainly valuable for those single-object queries that make use of property {@link helpers.Column Column.skip},
     * which makes the list of effective columns dynamic.
     *
     * @param {object|array} data
     * Data intended for the query - a single source object or an array of such objects.
     *
     * Passing in a non-object value will throw {@link external:TypeError TypeError} = `Invalid parameter 'data' specified.`
     *
     * @returns {boolean}
     *
     * - `true` - this `ColumnSet` can generate a valid query for the `data` specified
     * - `false` - trying to generate a query will inevitably throw an error, either because
     *   there are no effective columns or there is no data (when `data` is an empty array).
     */
    this.canGenerate = function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            var skip = $arr.countIf(this.columns, function (c) {
                return c.cnd || (c.skip && c.skip.call(data[i], c.prop || c.name));
            });
            if (skip === total) {
                return false;
            }
