...
function Column(col) {
    ...
    // TODO: detect invalid variable names (like with invalid mods);
    // TODO: detect invalid sql type castings (after removing ::);
    ...
    if (typeof col === 'string') {
        ...
        if (!$npm.utils.isText(this.name)) {
            throw new TypeError("A column name cannot be empty.");
        }
    } else {
        if (col && typeof col === 'object' && 'name' in col) {
            ...
            if (!$npm.utils.isNull(col.prop)) {
                if (!$npm.utils.isText(col.prop)) {
                    throw new TypeError("The value of 'prop' must be a non-empty text string.");
                }
                this.prop = col.prop; // optional property name (if different from the columns name);
            }
            if (!$npm.utils.isNull(col.mod)) {
                if (!$npm.utils.isText(col.mod)) {
                    throw new TypeError("Property 'mod' must be a non-empty text string.");
                }
                this.mod = col.mod; // optional format modifier;
            }
            if (!$npm.utils.isNull(col.cast)) {
                if (!$npm.utils.isText(col.cast)) {
                    throw new TypeError("Property 'cast' must be a non-empty text string.");
                }
            ...
            throw new TypeError("Invalid column details. It must be a string or a configurator object.");
        }
    }
