...
class ColumnSet extends InnerState {
    ...
    constructor(columns, opt) {
        ...
        opt = assert(opt, ['table', 'inherit', 'duplicate']);
 ...
 * // }
 */
ColumnSet.prototype.extend = function (columns, opts) {
    const duplicate = opts && opts.skip ? 'skip' : 'error';
    let cs = columns;
    if (!(cs instanceof ColumnSet)) {
        cs = new ColumnSet(columns, {duplicate});
    }
    return new ColumnSet(this.columns.concat(cs.columns), {table: this.table, duplicate});
};
