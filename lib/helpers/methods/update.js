...
function update(table, cols, obj, capSQL) {
    ...
    var fm = $npm.formatting.as;

    query = fm.format(query, table);

    var sets = cols.columns.map(function (c) {
        return fm.name(c.name) + '=${' + c.name + c.mod || '' + '}' + c.cast ? ('::' + c.cast) : '';
    }).join(',');

    return query + fm.format(sets, cols.prepare(obj));
}
