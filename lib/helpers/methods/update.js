 ...
 * with `table` set in it, that will be used when this parameter isn't specified. It is optional for single-object
 * updates, to allow generating updates that are part of an insert (`UPSERT` queries).
 *
 * If it is not available for a multi-object update, the method will throw {@link external:Error Error} = `Table name is unknown.`
 *
