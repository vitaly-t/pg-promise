 ...
 * with `table` set in it, that will be used when this parameter isn't specified. It is also optional for single-object
 * updates, to allow generating updates that are part of an insert (`UPSERT` queries).
 *
 * If it is not available during a multi-object update, the method will throw {@link external:Error Error} = `Table name is unknown.`
 *
 ...
 *
 * // Although column details can be taken from the data object, it is not a likely scenario
 * // one wants for an update, unless updating the whole table:
 *
 * var query = update(dataSingle, null, 'my-table');
 ...
 *
 * var query = update(dataSingle, ['val', 'msg'], 'my-table') + ' WHERE id = ' + dataSingle.id;
 ...
 *
 * var query = update(dataMulti, ['?id', 'val', 'msg'], 'my-table') + ' WHERE v.id = t.id';
 * //=> UPDATE "my-table" AS t SET "val"=v."val","msg"=v."msg" FROM (VALUES(1,123,'hello'),(2,456,'world!'))
 * //   AS v("id","val","msg") WHERE v.id = t.id
 *
 * @example
 *
 * // Using all details from a reusable ColumnSet (recommended for performance):
 *
 * var cs = new ColumnSet(['?id', 'val', 'msg'], {table: 'my-table'});
 *
 * var query = update(dataMulti, cs) + ' WHERE v.id = t.id';
 * //=> UPDATE "my-table" AS t SET "val"=v."val","msg"=v."msg" FROM (VALUES(1,123,'hello'),(2,456,'world!'))
 * //   AS v("id","val","msg") WHERE v.id = t.id
 *
 * @example
 ...
 *
 * var query = update(dataMulti, cs, null, {tableAlias: 'X', valueAlias: 'Y'}) + ' WHERE Y.id = X.id';
...
function update(data, columns, table, options, capSQL) {
    ...
    return format(query, table ? (' ' + table.name) : '') + format(updates, columns.prepare(data));

    function checkTable(optional) {
