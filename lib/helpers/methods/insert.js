 ...
 *
 */
function insert(columns, obj, table, capSQL) {
    ...
    if (Array.isArray(obj)) {
        var values = obj.map(function (o) {
            return '(' + fm(columns.variables, columns.prepare(o)) + ')';
        }).join(',');
        return fm(query + '$3^', [table, columns.names, values]);
    } else {
        var values = fm(columns.variables, columns.prepare(obj));
        return fm(query + '($3^)', [table, columns.names, values]);
    }
}
