...
var $events = {
     ...
     * @event finish
     * @description
     * Global notification of query execution has finished.
     *
     * The library will suppress any error thrown by the handler and write it into the console.
     *
     * @param {object} stats
     ...
     *  - rowCount: number of rows fetched/affected by a query
     *
     * @param {object} e
     * Event Context Object.
     *
     ...
     *
     * var options = {
     *     finish: function (stats, e) {
     *         console.log('Query: ' + e.query + ' has finished in: ' + stats.duration + ', rowCount: ' + stats.rowCount)
     *     }
     * };
     *
     */
    finish: function (options, stats, context) {
        if (typeof options.finish === 'function') {
            try {
                options.finish(stats, context);
            } catch (e) {
                // silencing the error, to avoid breaking the task;
                $events.unexpected('finish', e);
            }
        }
    },
