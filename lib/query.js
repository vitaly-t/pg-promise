...
function $query(ctx, query, values, qrm, config) {
    ...
    if (!isFile && query && typeof query === 'object' && !(query instanceof $npm.PS) && !(query instanceof $npm.PQ)) {
        if ('name' in query) {
            query = new $npm.PS(query);
        } else {
            if ('text' in query) {
                query = new $npm.PQ(query);
            }
        }
    }

    if (query instanceof $npm.PQ || query instanceof $npm.PS) {
        pgFormatting = true;
    }
