...
function $query(ctx, query, values, qrm, config) {
    ...
    var errMsg,
        ...
        isPS = !isFile && (query instanceof $npm.PS || $npm.utils.isObject(query, ['name', 'text'])), // prepared statement;
        opt = ctx.options,
            ...
            if (isPS) {
                if (query instanceof $npm.PS) {
                    ...
                    if (!$npm.utils.isText(query.name)) {
                        errMsg = "Invalid 'name' in a prepared statement.";
                    } else {
                        if (!$npm.utils.isText(query.text)) {
                            errMsg = "Invalid 'text' in a prepared statement.";
                        } else {
                            if (!$npm.utils.isNull(query.values) && !Array.isArray(query.values)) {
                                errMsg = "Invalid 'values' in a prepared statement.";
                            }
                        }
                    }
                }
