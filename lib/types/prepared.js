...
// TODO: Remove all short-syntax uses.
...
class PreparedStatement {
    ...
    parse() {
        ...
        const qf = _i.state.text instanceof npm.QueryFile ? _i.state.text : null;
        ...
        if (qf) {
            ...
            if (qf.error) {
                ...
                _i.PS.text = qf[npm.QueryFile.$query];
            }
        } else {
            _i.PS.text = _i.state.text;
        }
        ...
        if (errors.length) {
            return _i.currentError = new npm.errors.PreparedStatementError(errors[0], _i.PS);
        }
