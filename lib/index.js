...
function $stream(ctx, qs, init) {
    if (!qs || typeof(qs) !== 'object' || !('state' in qs)) {
        return $p.reject("Invalid stream object passed.");
    }
    if (qs._reading || qs.state !== 'initialized') {
        return $p.reject("Invalid stream state.");
    }
    if (typeof(init) !== 'function') {
        return $p.reject("Initialization function required.");
    }
    ...
    if (!error && !qs._reading) {
        // stream wasn't initialized by the callback function;
...
function $main(options) {
    ...
    // library version;
    inst.version = new Version(1, 8, 0);
