...
function Transaction(ctx, tag) {
    ...
    // sequentially resolve dynamic promises returned by a factory;
    this.sequence = this.queue = function (factory) {
        ...
        if (!errMsg) {
            ...
            if (!isRaw) {
                if (qrm === null || qrm === undefined) {
                    ...
                    if (typeof(qrm) !== 'number' || !isFinite(qrm) || (qrm & badMask) === badMask || qrm < 1 || qrm > 6) {
...
function $main(options) {
    ...
    // library version;
    inst.version = new Version(1, 6, 7);
