    ...
    function taskProcessor(p1, p2, isTX) {
        ...
        if (tag === undefined) {
            if (taskCtx.cb.tag !== undefined) {
                ...
                if (taskCtx.cb.name) {
                    tag = taskCtx.cb.name; // use the function name as tag;
                }
            }
        }
...
function $task(ctx, obj, isTX) {
    ...
    function update(start, success, result) {
        ...
        return (isTX ? $notify.transact : $notify.task)(ctx.options, {
            client: ctx.db.client,
            ctx: c
        });
    }
    ...
    if (isTX) {
        ...
        if (notifyError !== undefined) {
            return $p.reject(notifyError);
        }
        return callback()
            .then(function (data) {
                notifyError = update(false, true, data);
                if (notifyError === undefined) {
    ...
    task: function (options, context) {
        if (options && options.task instanceof Function) {
            try {
                ...
                // throwing an exception is ok during event 'task',
                // it will result in a proper reject for the task.
                return err;
            }
        }
    },
