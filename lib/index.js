...
function $extend(ctx, obj) {
     ...
     * @summary Executes a query that expects no return data.
     ...
     * @method one
     * @memberof Database.prototype
     * @summary Executes a query that expects exactly one row.
     * @param query{string|object} - query string or prepared statement object
     * @param values{array|value} - formatting parameters for the query string
     * @returns A promise object that represents the query result:
     * - when 1 record is returned, it will resolve with the row as a single object;
     * - when no records are returned, it will reject with `"No data returned from the query."`;
     * - when more than 1 records are returned, it will reject with
     ...
     * @param values{array|value} - formatting parameters for the query string
     * @returns A promise object that represents the query result:
     * - when 1 or more records are returned, it will resolve with an array of rows.
     * - when no records are returned, it will reject with `"No data returned from the query."`;
     */
    ...
    // - resolves with the row-object when 1 row returned;
...
// library version;
$main.version = new Version(1, 8, 6);
