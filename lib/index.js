...
function dbInit(cn, options) {
    ...
    dbInst.connect = function () {
        ...
        return $connect(cn)
            .then(function (obj) {
                ...
                db.done = function () {
                    $notify.disconnect(options, obj);
                    obj.done(); // release the connection;
                };
                $notify.connect(options, obj);
    ...
    function dbQuery(query, values, qrm, isRaw) {
        return $p(function (resolve, reject) {
            $connect(cn)
                .then(function (db) {
                    $notify.connect(options, db);
                    $query(db, query, values, qrm, options, isRaw)
                        .then(function (data) {
                            $notify.disconnect(options, db);
                            db.done();
                            resolve(data);
                        }, function (reason) {
                            $notify.disconnect(options, db);
...
function $extend(obj, cn, db, options) {
    ...
    obj.tx = function (p1, p2) {
        ...
        function attach(c, int) {
            ...
            if (internal) {
                $notify.connect(options, txDB); // notify of a new connection;
            }
        }
        ...
        function detach() {
            ...
            if (internal) {
                ...
                $notify.disconnect(options, txDB); // notify of disconnection;
...
var $notify = {
    connect: function (options, db) {
        if (isFunction(options, 'connect')) {
            options.connect(db.client);
        }
    },
    disconnect: function (options, db) {
        if (isFunction(options, 'disconnect')) {
            options.disconnect(db.client);
        }
    },
    ...
    transact: function (options, context) {
        if (isFunction(options, 'transact')) {
            try {
                ...
                // event, as it will break the transaction command flow;
            }
        }
    },
    error: function (options, err, context) {
        if (isFunction(options, 'error')) {
            try {
                ...
                // have to silence errors here:
                // throwing unhandled errors while handling
                // an error notification isn't acceptable.
            }
        }
    },
