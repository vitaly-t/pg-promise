...
function Task(ctx, tag, isTX) {
     ...
     * @param {function} factory - a callback function to create and return a new query
     * request, based on the request index passed.
     * @param {boolean} [noTracking=false] - defaults to `false`, when set to `true`, this parameter
     * prevents tracking resolve results from individual query requests, to avoid memory
     * overuse during super-massive transactions.
...
function $extend(ctx, obj) {
     ...
     * @param {object|function} p1 - a tag object, if it is not `undefined` and at least
     * two parameters are passed, or else it is the callback function for the task.
     * @param {function} [p2] - task callback function, but only if p1 was passed in also,
 ...
 */
$main.version = new Version(1, 9, 6);
