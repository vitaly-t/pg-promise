...
function $transact(obj, cb, inst) {
    ...
    function txUpdate(start, success, result) {
        if (start) {
            inst.ctx.start = new Date();
            delete inst.ctx.finish;
            delete inst.ctx.success;
            delete inst.ctx.result;
        } else {
            inst.ctx.finish = new Date();
            inst.ctx.success = success;
            inst.ctx.result = result;
        }
        if (inst.options && inst.options.transact) {
            var func = inst.options.transact;
            if (typeof(func) !== 'function') {
