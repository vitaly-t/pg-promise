...
function Library(options) {
    ...
    self._createDB = function (cn) {
        ...
        db.on('error', function () {
            // stop EventEmitter from turning Database `error` event
            // into `uncaughtException` when there are no handlers.
        });
        return db;
    };
    ...
    dbInst.connect = function () {
        ...
        var self = {
            query: function (query, values, qrm) {
                ...
                return $query(dbInst, cnDB, query, values, qrm, options);
            },
            done: function () {
                if (!cnDB) {
        ...
        $extend(dbInst, self, null, cnDB, options); // extending the protocol;
...
Library.prototype = Object.create(npm.ee.prototype);
Database.prototype = Object.create(npm.ee.prototype);
            ...
            if (err) {
                ...
                resolve({
                    client: client,
                        ...
                        $notify.disconnect(dbInst, options, client);
                    }
                });
                $notify.connect(dbInst, options, client);
            }
        });
    });
}
...
var $notify = {
    ...
    extend: function (dbInst, options, obj) {
        ...
        if (dbInst !== obj) {
            // Emitting the event only for transactions here,
            // as Library type emits it for the Database level.
