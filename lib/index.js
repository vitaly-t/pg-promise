...
function $sequence(t, factory) {
    ...
    function loop(factory, idx, result) {
        var e;
        try {
            e = factory(t, idx); // next promise;
        } catch (e) {
            return $p.reject(e.message || e);
        }
        if (!e) {
            // no more promise objects left;
            return $p.resolve(result);
        }
        if (typeof(e.then) !== 'function') {
            return $p.reject("Promise factory returned invalid result from index " + idx);
        }
        ...
        if (!errMsg) {
            ...
            if (!isRaw && ((qrm & badMask) === badMask || qrm < 1 || qrm > 6)) {
                ...
                if (!pgFormatting || isFunc) {
                    ...
                    try {
                        if (isFunc) {
                            query = npm.formatting.funcQuery(query, values);
                        } else {
        ...
        var tx = {
            ...
            sequence: function (factory) {
                if (typeof(factory) !== 'function') {
                    throw new Error("Invalid factory function passed.");
                }
