...
function $notify(open, db, opt) {
    if (opt) {
        ...
        if (func) {
            checkFunction(func, open ? 'connect' : 'disconnect');
            func(db.client);
        }
    }
}
...
function checkFunction(func, event) {
    if (typeof(func) !== 'function') {
        throw new Error("Function was expected for 'options." + event + "'");
    }
}
