...
function Database(cn, options) {
    ...
    this.connect = function () {
        ...
        $extend(cnCtx, cnInst); // extending the protocol;
        return $connect(cnCtx)
            .then(function (obj) {
                cnCtx.start(obj);
                return $p.resolve(cnInst);
            });
    };

    this.query = function (query, values, qrm) {
        var dbCtx = new Context(self, cn, options);
        return $connect(dbCtx)
            .then(function (session) {
                dbCtx.start(session);
                return $query(dbCtx, query, values, qrm)
                    .then(function (data) {
                        dbCtx.finish();
    ...
    $extend(ctx, self); // extending root protocol;
}

////////////////////////////
// Context type;
function Context(db, cn, options, session) {
    ...
    this.clone = function () {
        return new Context(this.db, this.cn, this.options, this.session);
    };
}

////////////////////////////
// Transaction type;
function Transaction(ctx, p1, p2) {
    ...
    if (tag !== undefined) {
        txCtx.ctx.tag = tag; // transaction tag;
    }
...
function $main(options) {
    ...
    // Library version;
    inst.version = new Version(1, 5, 0);
...
function $extend(ctx, obj) {
    ...
    function transaction(p1, p2) {
        return new Transaction(ctx, p1, p2);
    }
...
function $query(ctx, query, values, qrm) {
    return $p(function (resolve, reject) {
        ...
        if (!errMsg) {
            ...
            if (!errMsg) {
                ctx.session.client.query(query, params, function (err, result) {
                    var data;
                    if (err) {
                        errMsg = err;
                    } else {
