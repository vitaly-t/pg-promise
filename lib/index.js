...
module.exports = function (options) {
    ...
    if (lib) {
        ...
        if (t === 'function' || t === 'object') {
            // 'Promise' object is supported by libraries: Bluebird, When, Q, RSVP.
            // And libraries 'promise' and 'lie' use their main function instead.
...
function dbInit(cn, options) {
    ...
    dbInst.connect = function () {
        ...
        function cnQuery(query, values, qrm, raw) {
            if (!db.client) {
                throw new Error("Cannot execute a query on a disconnected client.");
            }
            return $query(db, query, values, qrm, options, raw);
        }
    ...
    function dbQuery(query, values, qrm, raw) {
        return $p(function (resolve, reject) {
            $connect(cn)
                .then(function (db) {
                    $notify(true, db, options);
                    $query(db, query, values, qrm, options, raw)
                        .then(function (data) {
...
function $query(db, query, values, qrm, options, raw) {
    return $p(function (resolve, reject) {
        ...
        if (typeof(query) !== 'string') {
            ...
            if (!raw && ((qrm & badMask) === badMask || qrm < 1 || qrm > 6)) {
                        ...
                        if (raw) {
                            ...
                            if (l) {
                                ...
                                if (qrm & queryResult.none) {
                                    ...
                                    errMsg = "No rows returned from the query.";
                                }
                            }
                        }
                    }
...
function $extend(obj, cn, db, options) {
    ...
    function queryFunc(funcName, values, qrm) {
        var query;
        try {
            query = npm.formatting.funcQuery(funcName, values);
        } catch (e) {
            $notifyError(e, options, {
                client: db.client,
                query: funcName,
                params: values,
                ctx: db.ctx
    ...
    obj.func = function (funcName, values, qrm) {
        return queryFunc(funcName, values, qrm);
    };
    ...
    obj.proc = function (procName, values) {
        return queryFunc(procName, values, queryResult.one | queryResult.none);
    };
    ...
    obj.tx = function (p1, p2) {
        ...
        function txQuery(query, values, qrm, raw) {
            if (!txDB.client) {
                throw new Error("Unexpected call outside of transaction.");
            }
            return $query(txDB, query, values, qrm, options, raw);
        }
