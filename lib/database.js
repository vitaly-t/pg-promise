 ...
 *
 */
function Database(cn, options, config) {
    ...
    function extend(ctx, obj) {
         ...
         * @description Custom data streaming, with the help of $[pg-query-stream].
         *
...
module.exports = function (config) {
    var npm = config.npm;
    npm.connect = npm.connect || require('./connect')(config);
    npm.query = npm.query || require('./query')(config);
    npm.task = npm.task || require('./task')(config);
    return Database;
};
