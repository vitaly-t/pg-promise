...
function Database(cn, dc, config) {
    ...
    function extend(ctx, obj) {
        ...
        /*
        * need to be able to call something like this inside extend:
        * const params = pgp.as.taskArgs(arguments, extraOptions);
        * 1) return obj.task.apply(obj, params);
        * 2) return obj.task.call(obj, params.options, params.cb);
        *
        * Problems with extraOptions: not clear when to override and when use as the defaults;
        * Solution: not to use it at all, let the app decide, based on what's set.
        * */
