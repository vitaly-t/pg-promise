...
function Database(cn, dc, config) {
    ...
    const dbThis = this, $p = config.promise;

    let pool = new config.pgp.pg.Pool(cn);
     ...
     * $[pg-pool] directly.
     *
     * Note that if you pass the pool object into a library that calls `pool.end()`, you will no longer be able to use this
     * {@link Database} object, and each query method will be rejecting with {@link external:Error Error} =
