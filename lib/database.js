...
function Database(cn, dc, config) {
    ...
    var pool = new config.pgp.pg.Pool(cn),
        $p = config.promise;
    ...
    this.connect = function (options) {
        ...
        var connection = (options && options.direct) ? config.$npm.connect.direct(ctx) : config.$npm.connect.pool(ctx, pool);
        return connection
            .then(db => {
                ctx.connect(db);
    ...
    this.query = function (query, values, qrm) {
        ...
        return config.$npm.connect.pool(ctx, pool)
            .then(db => {
                ctx.connect(db);
    ...
    /**
     * @member {} Database.$pool
     * @readonly
     * @description
     * ** This is a $[6.x] feature. **
     *
     * A $[pg-pool] object associated with the database object.
     *
     * This is a hidden property, to help integrating type {@link Database} directly with third-party libraries.
     *
     * @see Database
    ...
    $npm.utils.addReadProp(this, '$destroy', () => {
        if (!pool) {
            throw new Error('Invalid attempt at calling Database.$destroy()');
        }
    ...
    function extend(ctx, obj) {
        ...
        function taskProcessor(p1, p2, isTX) {
            ...
            // connection required;
            return config.$npm.connect.pool(taskCtx, pool)
                .then(db => {
                    taskCtx.connect(db);
